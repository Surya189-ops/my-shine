    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white overflow-x-hidden">
      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes pulse-glow { 0%, 100% { box-shadow: 0 0 20px rgba(236,72,153,0.4); } 50% { box-shadow: 0 0 60px rgba(236,72,153,0.8), 0 0 100px rgba(236,72,153,0.3); } }
        ...
        .card-hover:hover { transform: translateY(-8px); border-color: rgba(236,72,153,0.4) !important; }
        .btn-primary { background: linear-gradient(135deg, #ec4899, #be185d); transition: all 0.3s ease; position: relative; overflow: hidden; }
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 font-body ${scrolled ? "glass py-3" : "py-5"}`}>
        <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => router.push("/login")} className="text-sm text-gray-300 hover:text-white transition-colors px-4 py-2">Log In</button>
            <button onClick={() => router.push("/login")} className="btn-primary text-sm font-medium text-white px-5 py-2 rounded-full">Get Started</button>
          </div>
        </div>
      </nav>
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <button onClick={() => router.push("/login")} className="btn-primary font-body font-medium text-white text-base px-8 py-4 rounded-full w-full sm:w-auto">
              Start Connecting — Free ✨
            </button>
            <button onClick={() => router.push("/login")} className="font-body text-gray-300 hover:text-white text-base px-8 py-4 rounded-full border border-white/10 hover:border-white/30 transition-all w-full sm:w-auto">
              Start Chatting →
            </button>
          </div>
          <p className="font-body text-gray-600 text-sm mt-6">No credit card required • Free to join • Free to chat & connect</p>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              { step: "03", icon: "📹", title: "Chat & Video Call", desc: "Start chatting instantly and enjoy free video calls with people from around the world." },
            ].map((item) => (
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              { icon: "📹", title: "Free Video Calls",     desc: "Enjoy crystal clear video sessions with people from across the world — completely free" },
              ...
              { icon: "🌐", title: "Cross-Cultural",       desc: "Break language barriers and build real friendships across different cultures" },
            ].map((f) => (
        ...
        <div className="max-w-3xl mx-auto text-center">
          <div className="glass rounded-3xl p-12 border border-white/5 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="font-display text-4xl md:text-5xl font-bold text-white mb-4">Ready to Shine? ✨</h2>
              <p className="font-body text-gray-400 text-lg mb-8 max-w-xl mx-auto">
                Join thousands of users already connecting across borders. Free to start, free to use.
              </p>
              <button onClick={() => router.push("/login")} className="btn-primary font-body font-medium text-white text-base px-10 py-4 rounded-full">
                Create Free Account →
              </button>
            </div>
          </div>
